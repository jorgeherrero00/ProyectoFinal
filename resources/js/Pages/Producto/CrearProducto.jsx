...
const CrearProducto = ({ user }) => {
  ...
  const validateForm = () => {
    ...
    if (!state.price || isNaN(state.price) || state.price <= 1) {
      newErrors.price = 'El precio debe ser un número mayor que 1.';
    }
    if (!state.stock || isNaN(state.stock) || state.stock <= 1) {
      newErrors.stock = 'El stock debe ser un número mayor que 1.';
    }
    return newErrors;
  };
